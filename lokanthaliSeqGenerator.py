...
traffic_phase_state_L = 1
isPhaseRessetted_L = None

phase = {}
phase_number = None
def lokanthaliSeqGenerationFunction():
    global phase,phase_number, isPhaseRessetted_L, traffic_phase_state_L
    ...
    while True:
        ...
        if(isPhaseRessetted_L):
            tym = 0
            init_time = int(round(time.time()))

            if(traffic_phase_state_L==1):
                tym = phaseTime[0]

            if(traffic_phase_state_L == 2):
                tym = phaseTime[0] + phaseTime[1]

            if (traffic_phase_state_L == 3):
                tym = 0

            isPhaseRessetted_L = False
